<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (editMode && initialData) {
        // 수정 모드
        const updateData: UpdateTaskFlowDto = {
          name: name.trim(),
          sort,
          avail
        };
        await updateTaskFlow(initialData.code, updateData);
      } else {
        // 추가 모드
        const createData: CreateTaskFlowDto = {
          name: name.trim(),
          sort,
          avail
        };
        await createTaskFlow(createData);
      }
      ...
      error = editMode ? 'Task Flow 수정에 실패했습니다.' : 'Task Flow 추가에 실패했습니다.';
      console.error(e);
    } finally {
      loading = false;
    }
