<script lang="ts">
  ...
  import {
    createResultStatus,
    updateResultStatus,
    type CreateResultStatusDto,
    type UpdateResultStatusDto
  } from '$lib/api/result-statuses';
  import { Button } from '$lib/components/ui/button/index.js';
  import type { MPW_ASM_RESULT_STATUS } from '$lib/schemas/asm_result_status.schema';

  interface Props {
    editMode?: boolean;
    initialData?: MPW_ASM_RESULT_STATUS | null;
    onSuccess?: () => void;
    onCancel?: () => void;
  }
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (editMode && initialData) {
        // 수정 모드
        const updateData: UpdateResultStatusDto = {
          name: name.trim(),
          sort,
          avail
        };
        await updateResultStatus(initialData.code, updateData);
      } else {
        // 추가 모드
        const createData: CreateResultStatusDto = {
          name: name.trim(),
          sort,
          avail
        };
        await createResultStatus(createData);
      }
      ...
      error = editMode
        ? 'Result Status 수정에 실패했습니다.'
        : 'Result Status 추가에 실패했습니다.';
      console.error(e);
    } finally {
      loading = false;
    }
...
<form onsubmit={handleSubmit} class="space-y-4">
  ...
  <div class="form-row">
    <div class="form-group">
      <label for="sort">Sort <span class="required">*</span></label>
      <input
        id="sort"
        type="number"
        bind:value={sort}
