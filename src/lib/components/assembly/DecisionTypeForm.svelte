<script lang="ts">
  ...
  import {
    createDecisionType,
    updateDecisionType,
    type CreateDecisionTypeDto,
    type UpdateDecisionTypeDto
  } from '$lib/api/decision-types';
  import { Button } from '$lib/components/ui/button/index.js';
  import type { MPW_ASM_DECISION_TYPE } from '$lib/schemas/asm_decision_type.schema';

  interface Props {
    editMode?: boolean;
    initialData?: MPW_ASM_DECISION_TYPE | null;
    onSuccess?: () => void;
    onCancel?: () => void;
  }
  ...
  onMount(() => {
    if (editMode && initialData) {
      ...
      use_auto_sub = initialData.use_auto_sub;
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (editMode && initialData) {
        // 수정 모드
        const updateData: UpdateDecisionTypeDto = {
          name: name.trim(),
          use_auto_sub,
          avail
        };
        await updateDecisionType(initialData.code, updateData);
      } else {
        // 추가 모드
        const createData: CreateDecisionTypeDto = {
          name: name.trim(),
          use_auto_sub,
          avail
        };
        await createDecisionType(createData);
      }
      ...
      error = editMode
        ? 'Decision Type 수정에 실패했습니다.'
        : 'Decision Type 추가에 실패했습니다.';
      console.error(e);
    } finally {
      loading = false;
    }
...
<form onsubmit={handleSubmit} class="space-y-4">
  ...
  <div class="form-row">
    <div class="form-group">
      <label for="use_auto_sub">Use Auto Sub <span class="required">*</span></label>
      <select id="use_auto_sub" bind:value={use_auto_sub} disabled={loading} class="select-input">
